...
contract SymbolicAccount {
    ...
    function validatedLength() external view returns (uint256) {
        return validated_data.length;
    }

    function calledLength() external view returns (uint256) {
    ...
    function eq(uint256 index) external returns (bool) {
        ...
        for (uint256 i; i < called_data[index].length; i++) {
            if (called_data[index][i] != validated_data[index][i]) return false;
        }
        return true;
    }
